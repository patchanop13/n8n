            ...
            for (let i = 0; i < length; i++) {
                try {
                    if (operation === 'logo') {
                        ...
                        const response = yield GenericFunctions_1.brandfetchApiRequest.call(this, 'POST', `/logo`, body);
                        if (download === true) {
                            ...
                            for (const imageType of imageTypes) {
                                for (const imageFormat of imageFormats) {
                                    ...
                                    if (url !== null) {
                                        const data = yield GenericFunctions_1.brandfetchApiRequest.call(this, 'GET', '', {}, {}, url, { json: false, encoding: null });
                                        newItem.binary[`${imageType}_${imageFormat}`] = yield this.helpers.prepareBinaryData(data, `${imageType}_${domain}.${imageFormat}`);
                                        items[i] = newItem;
                                    }
                                    items[i] = newItem;
                                }
                    ...
                    if (operation === 'color') {
                        ...
                        const response = yield GenericFunctions_1.brandfetchApiRequest.call(this, 'POST', `/color`, body);
                        responseData.push(response.response);
                    }
                    if (operation === 'font') {
                        const domain = this.getNodeParameter('domain', i);
                        ...
                        const response = yield GenericFunctions_1.brandfetchApiRequest.call(this, 'POST', `/font`, body);
                        responseData.push(response.response);
                    }
                    if (operation === 'company') {
                        ...
                        const response = yield GenericFunctions_1.brandfetchApiRequest.call(this, 'POST', `/company`, body);
                        responseData.push(response.response);
                    }
                    if (operation === 'industry') {
                        ...
                        const response = yield GenericFunctions_1.brandfetchApiRequest.call(this, 'POST', `/industry`, body);
