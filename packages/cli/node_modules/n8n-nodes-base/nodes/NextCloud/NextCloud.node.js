            ...
            for (let i = 0; i < items.length; i++) {
                try {
                        ...
                        if (operation === 'download') {
                            ...
                            if (this.getNodeParameter('binaryDataUpload', i) === true) {
                                ...
                                if (item.binary === undefined) {
                                    throw new n8n_workflow_2.NodeOperationError(this.getNode(), 'No binary data exists on item!');
                                }
                                const propertyNameUpload = this.getNodeParameter('binaryPropertyName', i);
                                if (item.binary[propertyNameUpload] === undefined) {
                                    throw new n8n_workflow_2.NodeOperationError(this.getNode(), `No binary data property "${propertyNameUpload}" does not exists on item!`);
                                }
                    ...
                    if (['file', 'folder'].includes(resource)) {
                        ...
                        throw new n8n_workflow_2.NodeOperationError(this.getNode(), `The resource "${resource}" is not known!`);
                    }
                    ...
                    try {
                        responseData = yield GenericFunctions_1.nextCloudApiRequest.call(this, requestMethod, endpoint, body, headers, encoding, qs);
                    }
                    catch (error) {
                    ...
                    if (resource === 'file' && operation === 'download') {
                        ...
                        const jsonResponseData = yield new Promise((resolve, reject) => {
