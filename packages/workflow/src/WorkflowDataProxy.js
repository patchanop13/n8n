    ...
    getDataProxy() {
        ...
        const jmespathWrapper = (data, query) => {
            if (!Array.isArray(data) && typeof data === 'object') {
                return jmespath.search(Object.assign({}, data), query);
            }
            return jmespath.search(data, query);
        };
        const createExpressionError = (message, context) => {
            return new _1.ExpressionError(message, Object.assign({ runIndex: that.runIndex, itemIndex: that.itemIndex, failExecution: true }, context));
        };
        ...
        const base = Object.assign({ $: (nodeName) => {
                if (!nodeName) {
            ...
            }), $thisItem: that.connectionInputData[that.itemIndex], $binary: {}, $data: {}, $env: this.envGetter(), $evaluateExpression: (expression, itemIndex) => {
                ...
                return executionData;
            }, $json: {}, $node: this.nodeGetter(), $self: this.selfGetter(), $parameter: this.nodeParameterGetter(this.activeNodeName), $position: this.itemIndex, $runIndex: this.runIndex, $mode: this.mode, $workflow: this.workflowGetter(), $thisRunIndex: this.runIndex, $thisItemIndex: this.itemIndex, $now: luxon_1.DateTime.now(), $today: luxon_1.DateTime.now().set({ hour: 0, minute: 0, second: 0, millisecond: 0 }), $jmespath: jmespathWrapper, 
            // eslint-disable-next-line @typescript-eslint/naming-convention
            DateTime: luxon_1.DateTime,
            // eslint-disable-next-line @typescript-eslint/naming-convention
            Interval: luxon_1.Interval,
            // eslint-disable-next-line @typescript-eslint/naming-convention
            Duration: luxon_1.Duration }, that.additionalKeys);
