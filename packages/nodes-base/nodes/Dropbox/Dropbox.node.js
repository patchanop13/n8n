...
class Dropbox {
    ...
    execute() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let headers = {};
            let simple = false;
            const { accessType } = yield GenericFunctions_1.getCredentials.call(this);
            if (accessType === 'full') {
                // get the root directory to set it as the default for all operations
                const { root_info: { root_namespace_id } } = yield GenericFunctions_1.getRootDirectory.call(this);
                headers = {
            ...
            for (let i = 0; i < items.length; i++) {
                try {
                    ...
                    let responseData;
                    if (returnAll === true) {
                        responseData = yield GenericFunctions_1.dropboxpiRequestAllItems.call(this, property, requestMethod, endpoint, body, query, headers);
                    }
                    else {
                        responseData = yield GenericFunctions_1.dropboxApiRequest.call(this, requestMethod, endpoint, body, query, headers, options);
                    }
                    ...
                    if (resource === 'file' && operation === 'download') {
                        ...
                        items[i].binary[dataPropertyNameDownload] = yield this.helpers.prepareBinaryData(Buffer.from(responseData), filePathDownload);
                    }
                        ...
                        if (returnAll === true) {
                            returnData.push.apply(returnData, (simple === true) ? (0, GenericFunctions_1.simplify)(responseData) : responseData);
                        }
                        else {
                            returnData.push.apply(returnData, (simple === true) ? (0, GenericFunctions_1.simplify)(responseData[property]) : responseData[property]);
                        }
                    }
                    else {
