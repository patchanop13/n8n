...
function facebookApiRequest(method, resource, body = {}, qs = {}, uri, option = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        let credentials;
        if (this.getNode().name.includes('Trigger')) {
            credentials = yield this.getCredentials('facebookGraphAppApi');
        }
        else {
            credentials = yield this.getCredentials('facebookGraphApi');
        }
...
function getFields(object) {
    ...
    return [{ name: '*', value: '*' }].concat(data[object] || [])
        .map((fieldObject) => (Object.assign(Object.assign({}, fieldObject), { name: (fieldObject.value !== '*') ? (0, change_case_1.capitalCase)(fieldObject.value) : fieldObject.value })));
}
