            ...
            for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                ...
                if (itemIndex > 0 && options.batchSize >= 0 && options.batchInterval > 0) {
                    ...
                    if (itemIndex % batchSize === 0) {
                        yield new Promise(resolve => setTimeout(resolve, options.batchInterval));
                    }
                }
                                ...
                                if (options.bodyContentType === 'raw') {
                                    ...
                                    for (const propertyData of binaryPropertyNames) {
                                        ...
                                        if (propertyData.includes(':')) {
                                            ...
                                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'If more than one property should be send it is needed to define the in the format:<code>"sendKey1:binaryProperty1,sendKey2:binaryProperty2"</code>');
                                        }
                                        if (item.binary[binaryPropertyName] === undefined) {
                                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `No binary data property "${binaryPropertyName}" does not exists on item!`);
                                        }
                ...
                try {
                    let sendRequest = requestOptions; // tslint:disable-line:no-any
                ...
                if (authentication === 'genericCredentialType' ||
                    ...
                    requestPromises.push(this.helpers.requestWithAuthentication.call(this, nodeCredentialType, requestOptions, additionalOAuth2Options && { oauth2: additionalOAuth2Options }));
                }
            }
            // @ts-ignore
            const promisesResponses = yield Promise.allSettled(requestPromises);
            ...
            for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                ...
                if (responseFormat === 'file') {
                    ...
                    if (fullResponse === true) {
                        ...
                        newItem.binary[dataPropertyName] = yield this.helpers.prepareBinaryData(response.body, fileName);
                    }
                    else {
                        newItem.json = items[itemIndex].json;
                        newItem.binary[dataPropertyName] = yield this.helpers.prepareBinaryData(response, fileName);
                    }
